...
def _delete_all_under(parent_path):
    ...
    for obj in file_system_objects:
        ...
        if os.path.isdir(obj_path): # is a directory/folder
            size_sum += _delete_all_under(obj_path)
            size_sum += int(os.path.getsize(obj_path))
            os.rmdir(obj_path)
        else: # is a file
            size_sum += int(os.path.getsize(obj_path))
            os.remove(obj_path)
...
def delete_by_name_under_path(path_to_search, name_of_file_or_dir_to_delete, pattern):
    ...
    for obj in file_system_objects:
        ...
        if os.path.isdir(obj_path): # is a directory/folder
            folder_c, file_c, size_c = delete_by_name_under_path(obj_path, name_of_file_or_dir_to_delete, pattern)
    ...
    if os.path.exists(obj_path):
        ...
        if _is_dir and (pattern // 2 ) % 2 == 1: # target is a folder/directory AND pattern=1x
            ...
            size_sum += _delete_all_under(obj_path) # First, delete/remove all files and sub-folders under a specific sub-folder.
            size_sum += int(os.path.getsize(obj_path))
    ...
    return folder_count, file_count, size_sum
...
if __name__ == "__main__":
    ...
    print("folder_count, file_count, size_sum = delete_by_name_under_path(path_to_search, name_of_file_or_dir_to_delete, pattern)\n")
    print("path_to_search :: str")
    print("name_of_file_or_dir_to_delete :: str")
    print("pattern :: int")
    print("folder_count, file_count, size_sum :: int, int, int\n---")
